...
# Данный пример поочерёдно включает и выключает каналы модуля.      # * Строки со звёздочкой являются необязательными.
                                                                    #
from Py_iarduino_I2C_Relay import *                                 #   Подключаем библиотеку для работы с реле и силовыми ключами.
from time import sleep                        # Подключаем метод sleep библиотеки time
relay = Py_iarduino_I2C_Relay(0x09);                          #   Создаём объект relay для работы с функциями и методами библиотеки Py_iarduino_I2C_Relay, указывая адрес модуля на шине I2C.
