...
//  Параметр:               номер канала (1,2,3,4,ALL_CHANNEL).
uint8_t iarduino_I2C_Relay::digitalRead(uint8_t channel)
        {                                                           
        //  Если корректно указан номер канала, то ...
        if (((channel > 0) && (channel <= 4))
            || (channel==ALL_CHANNEL)) {
                //  Если реле/ключ был инициализирован, то ...
                if (valAddr){
                        //  Если уровень доступа модуля позволяет
                        //  чтение/запись цифровых значений, то ...
                        if (valAccess >= 1) {
                        //  Читаем значение регистра «REG_DIGITAL_ALL»:
                                _readBytes(REG_DIGITAL_ALL, 1);
                                //  Если указан один канал, то сбрасываем 
                                //  все биты не относящиеся к указанному каналу:
                                if (channel != ALL_CHANNEL) {
                                        // Сбрасываем биты
                                        data[0] &= (1 << (channel - 1));
                                        // Сдвигаем вправо для получения 0 или 1
                                        data[0] >>= channel - 1;
                                }
                                //  Возвращаем результат:
                                //  Возвращаем 4 младших бита.
                                return data[0] & 0x0F;
                        }
                }
        }
...
bool    iarduino_I2C_Relay::enableWDT               (uint8_t time){                                                     //  Параметр:               время в секундах от 1 до 254).
            ...
            return _writeBytes( REG_WDT, 1 );                                                                           //  Записываем 1 байт данных из массива «data» в регистр «REG_WDT».
}                                                                                                                       //
                                                                                                                        //
...
bool    iarduino_I2C_Relay::getStateWDT             (void){                                                             //  Параметр:               отсутствует.
            ...
            _readBytes( REG_WDT, 1 );                                                                                   //  Читаем 1 байт данных из массива регистра «REG_WDT» в массив «data».
